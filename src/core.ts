...
function getRecipeUsageLabel(line) {
  ...
  if (line.milliliters) return `${line.milliliters}ml`;
...
function convertRecipeLineToStockUnits(line) {
  const ingredient = ingredientById(line.ingredientId);
  const unitType = unitTypeDefinition(ingredient?.unitType || ingredient?.unit);
  if (line.grams) return unitType.id === "kilograms" ? line.grams / 1000 : line.grams;
  if (line.milliliters) return unitType.id === "liters" ? line.milliliters / 1000 : line.milliliters;
  return line.units || 0;
}
...
function buildRecipeLine(ingredientId, quantity, measureKey) {
  ...
  if (measureKey === "grams") return { ingredientId, grams: amount };
  if (measureKey === "milliliters") return { ingredientId, milliliters: amount };
...
function renderSellableProductForm() {
  ...
  if (!form || !categorySelect || !stationSelect || !vatSelect || !availabilityChecks || !ingredientSelect || !measureSelect || !draftPanel || !addRecipeButton || !createButton) return;
...
function syncOrderStatus(orderId) {
  ...
  if (tickets.every((ticket) => ticket.status === "Done")) order.status = "Done";
  else if (tickets.every((ticket) => ticket.status === "Ready" || ticket.status === "Done")) order.status = "Ready";
  else if (tickets.some((ticket) => ticket.status === "Preparing")) order.status = "Preparing";
...
export function initApp(): void {
  document.addEventListener("click", (event) => {
    ...
    if (removeDraft) removeOrderDraftLine(removeDraft.dataset.removeDraft);
