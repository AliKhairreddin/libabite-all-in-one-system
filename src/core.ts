...
function qrReedSolomonTables() {
  if (qrReedSolomonTables.cache) return qrReedSolomonTables.cache;
...
function recipeLineAppliesToOrder(line, orderContext = DEFAULT_RECIPE_ORDER_CONTEXT) {
  if (line.appliesTo !== "takeawayDelivery") return true;
...
function getRecipeUsageLabel(line) {
  ...
  if (line.grams) return `${line.grams}g${wasteLabel}`;
  if (line.milliliters) return `${line.milliliters}ml${wasteLabel}`;
  return `${line.units} pcs${wasteLabel}`;
}

function getRecipeMeasure(line) {
  if (line.grams !== undefined) return { key: "grams", label: "grams", shortLabel: "g", step: 5 };
  if (line.milliliters !== undefined) return { key: "milliliters", label: "milliliters", shortLabel: "ml", step: 5 };
...
function convertRecipeLineToStockUnits(line) {
  ...
  if (line.grams) return (unitType.id === "kilograms" ? line.grams / 1000 : line.grams) * multiplier;
  if (line.milliliters) return (unitType.id === "liters" ? line.milliliters / 1000 : line.milliliters) * multiplier;
...
function getStockRequirementsForItems(items, orderContext = DEFAULT_RECIPE_ORDER_CONTEXT) {
  ...
  normalizeOrderItems(items).forEach((item) => {
    ...
    (product.recipe || []).forEach((line) => {
      if (!recipeLineAppliesToOrder(line, orderContext)) return;
      const ingredient = ingredientById(line.ingredientId);
      if (!ingredient) return;
...
function getProductAvailability(product, reservedItems = state.orderDraft, orderContext = DEFAULT_RECIPE_ORDER_CONTEXT) {
  if (!product) return { maxQuantity: 0, limiting: null, details: [] };
  if (!product.active) return { maxQuantity: 0, limiting: null, details: [] };
...
function renderProductsInSelects() {
  ...
  if (fulfillmentInput) fulfillmentInput.value = orderType.fulfillment;
...
function renderOrderBuilder() {
  ...
  if (orderForm?.elements.fulfillment) orderForm.elements.fulfillment.value = orderType.fulfillment;
...
function getOrderProgressSummary(order) {
  ...
  else if (total && completed === total) status = "Complete";
  else if (total && finished === total) status = "Ready";
  else if (preparing) status = "Preparing";
  else if (accepted) status = "Accepted";
  else if (finished) status = "In progress";
  else if (total) status = "New";
...
function procedureStatusClass(status) {
  ...
  if (status === "Skipped" || status === "Missed") return "warning";
  return "info";
}

function procedureFrequencyWindowMs(frequency) {
  if (frequency === "Weekly") return 7 * 24 * 60 * MINUTE_MS;
  if (frequency === "Monthly") return 31 * 24 * 60 * MINUTE_MS;
  if (frequency === "Per shift") return 12 * 60 * MINUTE_MS;
...
function procedureAssignmentAliases(user = currentUser()) {
  ...
  if (user.role === "owner_admin") aliases.add("Owner/Admin");
  if (user.role === "manager") aliases.add("Manager");
  if (user.role === "waiter_cashier") {
    aliases.add("Front");
    aliases.add("Cashier");
  }
  if (user.role === "kitchen_staff") aliases.add("Kitchen");
  if (user.role === "driver") aliases.add("Driver");
  return aliases;
}

function procedureAssignedToUser(procedure, user = currentUser()) {
  if (!user) return false;
  if (can("canReviewProcedures")) return true;
...
function advanceStatus(current) {
  if (current === "Delayed") return "Preparing";
...
function setTicketStatus(ticket, status) {
  ...
  if (status === "Ready" && !ticket.readyAtMs) {
    if (!ticket.startedAtMs) ticket.startedAtMs = now;
    ticket.readyAtMs = now;
  }

  if (status === "Done" && !ticket.completedAtMs) {
    if (!ticket.startedAtMs) ticket.startedAtMs = now;
    if (!ticket.readyAtMs) ticket.readyAtMs = now;
...
export function initApp(): void {
  ...
  document.querySelector("#resetDemoBtn").addEventListener("click", () => {
    ...
    if (state.users.some((user) => user.id === previousUserId)) state.currentUserId = previousUserId;
    saveState();
    render();
