...
export function createInventoryActionsRuntime(deps) {
  ...
  function selectSupplierForEdit(supplierId) {
    if (!can("canManageInventory")) return;
  ...
  function clearSupplierForm() {
    ...
    const form: any = document.querySelector("#supplierForm");
    if (form) {
      form.reset();
  ...
  function saveSupplierRecord(formData) {
    ...
    productsSupplied.forEach((ingredientId) => {
      const ingredient = ingredientById(ingredientId);
      if (ingredient) ingredient.supplier = name;
    });
  ...
  function getSupplierOrderQuantityInput(orderId, ingredientId, fallback) {
    ...
    const input: any = [...document.querySelectorAll("[data-supplier-order-quantity]")]
      .find((element: any) => element.dataset.supplierOrderQuantity === key);
  ...
  function getSupplierReceivedQuantityInput(orderId, ingredientId, fallback) {
    const key = `${orderId}::${ingredientId}`;
    const input: any = [...document.querySelectorAll("[data-supplier-received-quantity]")]
      .find((element: any) => element.dataset.supplierReceivedQuantity === key);
    return normalizeStockQuantity(input?.value ?? fallback);
  }
  ...
  function approveSupplierOrder(orderId) {
    ...
    if (supplier?.autoSendAfterApproval) markSupplierOrderSent(order);
  ...
  function markSupplierOrderOrdered(supplier) {
    ...
    if (order.status === "Approved") sendSupplierOrder(order.id);
  }
