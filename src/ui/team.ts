...
export function createTeamUi(deps) {
  ...
  function staffShiftCard(shift, options: any = {}) {
  ...
  function renderTimeClock() {
    ...
    const user = currentUser();
    if (!panel || !summary || !list || !user) return;
  ...
  function renderShiftFormOptions() {
    const staffSelect: any = document.querySelector("#shiftStaffSelect");
    const roleSelect: any = document.querySelector("#shiftRoleSelect");
    const stationSelect: any = document.querySelector("#shiftStationSelect");
    const dateInput: any = document.querySelector("#shiftDateInput");
    if (!staffSelect || !roleSelect || !stationSelect || !dateInput) return;
    ...
    staffSelect.innerHTML = staffOptions
      ...
      .join("");
    if (!staffOptions.some((account) => account.id === staffSelect.value)) staffSelect.value = staffOptions[0]?.id || "";

    const selectedRole = roleSelect.value || "Front";
    roleSelect.innerHTML = scheduleRoleOptionsHtml(selectedRole);
    if (!SCHEDULE_ROLES.includes(roleSelect.value)) roleSelect.value = "Front";
    ...
    if (!SCHEDULE_STATIONS.includes(stationSelect.value)) stationSelect.value = "Restaurant floor";
  ...
  function renderScheduleManagement() {
    ...
    const payrollExport: any = document.querySelector("#payrollExportPreview");
    if (!panel || !summary || !grid || !reportList || !payrollExport) return;
