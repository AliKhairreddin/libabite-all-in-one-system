...
import {
  ...
  normalizePaymentStatus as normalizeLedgerPaymentStatus,
...
export function normalizeExternalPlatforms(platforms) {
  ...
  function upsertPlatform(source: any = {}) {
...
export function normalizeExternalProductMappings(mappings, productIds, platforms = []) {
  ...
  function upsertMapping(mapping: any = {}, index = 0) {
    ...
    if (!productIds.has(productId) || (!externalName && !externalCode)) return;
...
export function normalizeExternalOrderImports(imports, productIds, platforms = []) {
  ...
  return (Array.isArray(imports) ? imports : [])
    .map((record, index) => {
      ...
      if (!platformIds.has(platformId)) return null;
      return {
...
function normalizeReceiptPrintJob(job, index, printerSettings, orderIds) {
  ...
  if (orderId && orderIds && !orderIds.has(orderId)) return null;
...
export function normalizeState(candidate) {
  ...
  const seedIngredientById = new Map<string, AnyRecord>(normalizedSeedIngredients.map((ingredient: AnyRecord) => [ingredient.id, ingredient]));
  next.ingredients = normalizeIngredients(next.ingredients);
  next.ingredients.forEach((ingredient: AnyRecord) => {
    const seedIngredient = seedIngredientById.get(ingredient.id);
    if (!ingredient.barcode && seedIngredient?.barcode) ingredient.barcode = seedIngredient.barcode;
  });
  ...
  if (!next.reservations.some((reservation) => reservation.id === next.websiteLastReservationId)) next.websiteLastReservationId = "";
  if (!next.reservations.some((reservation) => reservation.id === next.reservationEditingId)) next.reservationEditingId = "";
  ...
  next.orders.forEach((order) => {
    ...
    if (ledgerInput) next.payments = upsertPaymentLedgerRecord(next.payments, ledgerInput);
  });
