...
export const COMMUNICATION_RECORD_TYPES = ["order", "reservation"] as const;
...
export type CommunicationRecordType = typeof COMMUNICATION_RECORD_TYPES[number];
...
const EVENT_RECORD_TYPES: Record<CommunicationEventType, CommunicationRecordType> = {
...
function rawRecord(record: any) {
  return record?.raw && typeof record.raw === "object" ? record.raw : record || {};
}

function recordValue(record: any, ...keys: string[]) {
...
function isOneOf(value: unknown, choices: string[]) {
...
export function normalizeCommunicationEmail(value: unknown) {
  ...
  if (local.length > 64 || !domain.includes(".") || domain.startsWith(".") || domain.endsWith(".")) return "";
  return email;
}

export function isCommunicationRecordType(value: unknown): value is CommunicationRecordType {
  return COMMUNICATION_RECORD_TYPES.includes(value as CommunicationRecordType);
}

export function isCommunicationEventType(value: unknown): value is CommunicationEventType {
  return COMMUNICATION_EVENT_TYPES.includes(value as CommunicationEventType);
}

export function getRecordRecipient(recordType: CommunicationRecordType, record: any) {
  const email = normalizeCommunicationEmail(
    recordType === "order"
...
export function hasExplicitMarketingConsent(record: any) {
  const raw = rawRecord(record);
  return record?.marketingConsent === true || raw?.marketingConsent === true;
}

export function getMarketingConsentEvidence(record: any) {
  const raw = rawRecord(record);
  const timestampValue = recordValue(
    record,
    "marketingConsentAtMs",
    "marketingConsentedAtMs",
    "marketingConsentAt",
  ...
  const policyVersion = cleanText(
    record?.marketingConsentPolicyVersion
...
export function validateCommunicationEvent(recordType: CommunicationRecordType, eventType: unknown, record: any) {
  if (!isCommunicationEventType(eventType)) {
    return { ok: false as const, reason: "Unsupported communication event." };
  }
  if (EVENT_RECORD_TYPES[eventType] !== recordType) {
    return { ok: false as const, reason: `${eventType} cannot be used for a ${recordType}.` };
  }
...
export function communicationEventFingerprint(recordType: CommunicationRecordType, eventType: CommunicationEventType, record: any) {
...
function orderLifecycleIsCancelled(record: any) {
 ...
 */
export function communicationEventRemainsCurrent(
  recordType: CommunicationRecordType,
  eventType: CommunicationEventType,
  record: any,
  queuedVariables?: Partial<CommunicationTemplateVariables> | null
) {
  if (recordType === "reservation") {
    if (!validateCommunicationEvent(recordType, eventType, record).ok) return false;
    if (!queuedVariables) return true;
  ...
  switch (eventType) {
    ...
    case "order.ready":
      return !cancelled && (
        isOneOf(status, ["ready", "served", "completed"])
...
export function communicationEventIsCurrent(
  appStateKey: string,
  recordType: CommunicationRecordType,
  recordId: string,
  eventType: CommunicationEventType,
  record: any,
  queuedDedupeKey: string,
  queuedVariables?: Partial<CommunicationTemplateVariables> | null
) {
  if (!communicationEventRemainsCurrent(recordType, eventType, record, queuedVariables)) return false;
  if (queuedVariables) return true;
...
export function marketingConsentProviderStatusAfterIntegration(
  currentStatus: unknown,
  providerStatus: unknown,
  explicitMarketingConsent: boolean
) {
  return explicitMarketingConsent ? cleanText(providerStatus) : cleanText(currentStatus);
}

export function marketingIntegrationDedupeKey(
  appStateKey: string,
  recordType: CommunicationRecordType,
  recordId: string,
  email: string,
...
export function getCommunicationTemplateVariables(
  recordType: CommunicationRecordType,
  recordId: string,
  record: any,
...
function orderReference(variables: CommunicationTemplateVariables) {
  return `order #${variables.recordNumber || variables.recordId}`;
}

function reservationReference(variables: CommunicationTemplateVariables) {
  return [variables.date, variables.time].filter(Boolean).join(" at ") || "your requested time";
}

function messageSummary(eventType: CommunicationEventType, variables: CommunicationTemplateVariables) {
...
function detailLines(variables: CommunicationTemplateVariables) {
...
export function buildTransactionalMessage(eventType: CommunicationEventType, variables: CommunicationTemplateVariables) {
