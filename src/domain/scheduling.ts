...
export function combineShiftDateTimeMs(dateString, timeString, options: any = {}) {
  const date = normalizeShiftDate(dateString);
  const time = normalizeShiftTime(timeString);
  const timestamp = new Date(`${date}T${time}:00`).getTime();
  if (!options.afterMs || timestamp > options.afterMs) return timestamp;
...
export function getShiftAttendanceStatus(shift, nowMs = Date.now()) {
  ...
  if (shift?.breakStartedAtMs) return "On break";
  if (shift?.clockInAtMs) return getShiftLateMinutes(shift) ? "Late" : "On shift";
  return shift?.notifiedAtMs ? "Notified" : "Scheduled";
}

export function getDriverOnTimeStatus(shift, nowMs = Date.now()) {
  if (normalizeScheduleRole(shift?.role, "") !== "Driver") return "";
  if (shiftIsMissed(shift, nowMs)) return "Missed";
  if (!shift?.clockInAtMs) return "Pending";
