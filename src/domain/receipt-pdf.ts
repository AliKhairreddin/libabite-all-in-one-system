...
export interface ReceiptPdfTotal {
  label: string;
  value: string;
}
...
export function createReceiptPdfBlob(input: ReceiptPdfInput) {
  const textRows: Array<{ value?: string; size?: number; bold?: boolean; gapAfter?: number; divider?: boolean }> = [
  ...
  input.items.forEach((item) => {
    ...
    if (item.detail) textRows.push(...wrapText(item.detail, 31).map((value) => ({ value, size: 7 })));
  });
