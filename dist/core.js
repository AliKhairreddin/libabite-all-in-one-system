...
import { MINUTE_MS, TICKET_STATUSES, UNPAID_PAYMENT_METHOD, DEFAULT_PAID_PAYMENT_METHOD, VAT_RATES, TICKET_SLA_MINUTES, SLA_WARNING_WINDOW_MINUTES, KITCHEN_STATIONS, PRODUCT_CATEGORIES, VAT_OPTIONS, AVAILABILITY_OPTIONS, UNIT_TYPES, DEFAULT_INVENTORY_LOCATIONS, INVENTORY_ACTIONS, DEFAULT_MARGIN_TARGET, DEFAULT_MARGIN_MINIMUM, DEFAULT_RECIPE_ORDER_CONTEXT, CUSTOMER_QR_CHANNEL, CUSTOMER_QR_ORDER_CONTEXT, WEBSITE_ORDER_CHANNEL, WEBSITE_PAYMENT_PROCESSOR, QR_CODE_STATUSES, DRIVER_IDLE_STATUS, ROLE_DEFINITIONS, LANGUAGE_OPTIONS, PROCEDURE_COMPLETION_STATUSES, DEFAULT_RESTAURANT_SETTINGS } from "./shared/constants.js";
import { saveState, state } from "./app/state.js";
import { createAppRenderer } from "./app/render.js";
import { normalizeKitchenStation, unitTypeDefinition, normalizeMarginPercent, normalizeRecipeWastePercent, normalizeRecipeAppliesTo, normalizeRecipeLine, normalizeRecipeLines, normalizeStockQuantity, normalizeInventoryLocationName, isDefaultInventoryLocation, sortInventoryLocations, getIngredientTotalStock, getIngredientPrimaryLocation, syncIngredientStock, normalizeWasteReason, getWasteUnitOptionsForIngredient, normalizeWasteUnitType, convertWasteQuantityToStockUnits, getWasteCost, normalizeRestaurantSettings, normalizeProcedureLanguage, normalizeProcedureFrequency, normalizeProcedureAssignedRole, normalizeProcedureDepartment, normalizeListInput, normalizeProcedureSteps, normalizeProcedureMedia, normalizeLineModifiers } from "./data/normalize.js";
import { deliveryIsLate, getDeliveryStatus, isActiveDelivery, isDeliveryOrder, normalizeDriverDeliveryStatus, normalizePickupStatus, setDriverIdle, syncDriverWithDeliveryOrder } from "./domain/delivery.js";
...
import { getProductAvailability as getProductAvailabilityFromInventory, getStockRequirementsForItems as getStockRequirementsForItemsFromInventory, getStockShortages as getStockShortagesFromInventory, planStockDeduction } from "./domain/inventory.js";
import { advanceStatus, getOrderProgressSummary as summarizeOrderProgress, setTicketStatus } from "./domain/kitchen.js";
import { calculateItemsTotal, calculateOrderTotal, countOrderItems, normalizeOrderItems as normalizeOrderItemsForProducts, normalizeOrderFulfillment, normalizeOrderType, normalizeWebsiteFulfillment, orderTypeDefinition, productCanBeOrdered, productCanBeOrderedForOrderContext } from "./domain/orders.js";
import { getPaymentStatusForMethod, isPaidPaymentMethod, normalizePaymentMethod } from "./domain/payments.js";
import { getProductCost as getProductCostFromRecipe, getProductGrossMargin as getProductGrossMarginFromRecipe, getProductMargin as getProductMarginFromRecipe, getProductMarginProfile as getProductMarginProfileFromRecipe, productAvailabilityLabel as productAvailabilityLabelForProduct, productHasConditionalRecipeLines as productHasConditionalRecipeLinesForProduct } from "./domain/products.js";
import { convertActualUsageToStockUnits as convertActualUsageToStockUnitsForRecipe, convertRecipeLineToStockUnits as convertRecipeLineToStockUnitsForRecipe, getRecipeLineQuantity as getRecipeLineQuantityFromRecipe, getRecipeLineWasteMultiplier as getRecipeLineWasteMultiplierFromRecipe, getRecipeMeasure as getRecipeMeasureFromRecipe, getRecipeUsageLabel as getRecipeUsageLabelFromRecipe, isTakeawayDeliveryContext as isTakeawayDeliveryOrderContext, recipeLineAppliesToOrder as recipeLineAppliesToOrderContext } from "./domain/recipes.js";
import { getAvailableReservationTable as getAvailableReservationTableFromList, getReservationConflicts as getReservationConflictsFromList, getReservationIssues as getReservationIssuesFromList, getReservationValidation as getReservationValidationFromList, getReservationWindowLabel, isReservationTime } from "./domain/reservations.js";
...
import { formatDateTime, formatDuration, normalizeOptionalTimestamp, timeNow } from "./shared/dates.js";
...
function getProductionStepCheckboxes(form = document.querySelector("#productionForm")) {
    return Array.from(form?.querySelectorAll("[data-production-step]") || []);
}
...
function getOrderFulfillmentMeta(order) {
    ...
    if (order.requestedTime)
        details.push(`${fulfillmentLabel(order)} ${order.requestedTime}`);
    if (order.customerPhone)
        details.push(`Phone: ${order.customerPhone}`);
    if (order.deliveryAddress)
        details.push(`Address: ${order.deliveryAddress}`);
    const driver = driverById(order.assignedDriver);
    if (driver)
        details.push(`Driver: ${driver.name}`);
    const deliveryStatus = getDeliveryStatus(order);
    if (deliveryStatus)
        details.push(`Delivery: ${deliveryStatus}`);
    if (order.paymentReference)
        details.push(`Payment ref: ${order.paymentReference}`);
    return details;
}
...
function getSelectedLineModifiers() {
    const checked = Array.from(document.querySelectorAll("input[name='lineModifier']:checked"))
...
function processWebsiteOnlinePayment(formData, amount) {
    ...
    if (amount <= 0)
        return { ok: false, message: "Add a paid item before checkout." };
    if (cardName.length < 2)
        return { ok: false, message: "Enter the cardholder name." };
    if (cardNumber.length < 12 || cardNumber.length > 19)
        return { ok: false, message: "Enter a valid card number." };
    if (!/^\d{2}\/?\d{2}$/.test(expiry))
        return { ok: false, message: "Enter the card expiry as MM/YY." };
