...
export function createSettingsUi(deps) {
    ...
    const { can, emptyState, getAvailableReservationTable, getQrOrderUrl, getReservationIssues, getReservationValidation, getReservationWindowLabel, getWebsiteReservationUrl, tableById } = deps;
    ...
    function renderReservationPlanner() {
        ...
        document.querySelectorAll("#reservationBlockForm input[name='date']").forEach((input) => {
            if (!input.value)
                input.value = today;
        });
        if (title)
            title.textContent = editingReservation ? "Edit reservation" : "Add reservation";
        if (cancelButton)
            cancelButton.hidden = !editingReservation;
        const websiteReservationUrl = getWebsiteReservationUrl ? getWebsiteReservationUrl() : "?reservation=website";
        if (publicReservationInput)
            publicReservationInput.value = websiteReservationUrl;
        if (publicReservationLink)
            publicReservationLink.href = websiteReservationUrl;
    ...
    `;
        ...
        form.querySelectorAll("input, select, textarea, button").forEach((element) => {
            if (element.id === "publicReservationLinkInput")
                return;
            element.disabled = !editable;
        });
