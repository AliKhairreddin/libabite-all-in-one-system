...
import { LINE_MODIFIER_OPTIONS, ORDER_TYPE_OPTIONS, PAYMENT_METHOD_OPTIONS, PHONE_MESSAGE_FULFILLMENT_OPTIONS } from "../shared/constants.js";
...
export function createOrderBuilderUi(deps) {
    const { can, customerById, emptyState, findCustomerByPhone, findCustomerBySearchValue, formatStockAmount, fulfillmentLabel, getAddressHistoryForCustomer, getCurrentOrderContext, getCustomerOptionLabel, getCustomerPrimaryAddress, getDefaultProductionProductId, getFavoriteItemsForCustomer, getItemCount, getItemsTotal, getOrderTotal, getOrderableProductsForContext, getOrdersForCustomer, getProductAvailability, getProductionProducts, getStockShortages, money, normalizeOrderItems, productById, renderProductionRecipeFields } = deps;
    ...
    function getManualCustomerFromForm(form = document.querySelector("#orderForm")) {
        if (!form)
            return null;
    ...
    function customerHistoryHtml(customer) {
        if (!customer)
            return emptyState("Search a customer or enter a saved phone number to show history.");
    ...
    function renderManualOrderControls() {
        ...
        const driverSelect = document.querySelector("#manualDriverSelect");
        if (!form || !panel)
            return;
        ...
        panel.hidden = !manual;
        if (customerLabel)
            customerLabel.hidden = manual;
        if (fulfillmentLabelElement)
            fulfillmentLabelElement.hidden = !manual;
        if (addressLabel) {
            const addressText = addressLabel.querySelector("span");
            const addressInput = addressLabel.querySelector("textarea");
            if (addressText)
                addressText.textContent = isDelivery ? "Delivery address" : "Address history";
            if (addressInput)
                addressInput.placeholder = isDelivery ? "Street, number, city" : "Optional saved address";
        }
        if (driverLabel)
            driverLabel.hidden = !isDelivery;
        if (form.elements.customer)
            form.elements.customer.disabled = manual;
        panel.querySelectorAll("input, select, textarea").forEach((element) => {
            element.disabled = !manual || !editable;
        });
        if (form.elements.fulfillment)
            form.elements.fulfillment.disabled = !manual || !editable;
        if (!manual)
            return;
      ...
      `;
        ...
        if (historyPanel)
            historyPanel.innerHTML = customerHistoryHtml(getManualCustomerFromForm(form));
    }
    ...
    function renderOrderBuilder() {
        ...
        if (orderForm?.elements.fulfillment)
            orderForm.elements.fulfillment.value = fulfillment;
