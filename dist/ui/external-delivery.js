...
import { EXTERNAL_DELIVERY_IMPORT_METHODS, EXTERNAL_DELIVERY_PLATFORM_STATUSES } from "../shared/constants.js";
import { externalImportMethodLabel, externalPlatformName, normalizeExternalPlatformId } from "../domain/external-delivery.js";
import { escapeHtml } from "../shared/html.js";
export function createExternalDeliveryUi(deps) {
    const document = window.document;
    const { can, emptyState, formatDateTime, getOrderTotal, money, orderById, productById } = deps;
    ...
    function renderExternalPlatformForm() {
        ...
        if (!form || !platformSelect || !statusSelect || !methodSelect)
            return;
    ...
    function renderExternalMappingForm() {
        ...
        if (!form || !platformSelect || !productSelect)
            return;
        const editable = can("canEditSettings");
        ...
        if (!form.elements.externalName.value)
            form.elements.externalName.value = "Sandwich Kefta";
        if (!form.elements.externalCode.value)
            form.elements.externalCode.value = "99301";
        if (!form.elements.commissionRate.value)
            form.elements.commissionRate.value = "";
    ...
    function renderExternalOrderImportForm() {
        ...
        if (!form || !platformSelect || !methodSelect)
            return;
        ...
        if (!form.elements.commissionRate.value)
            form.elements.commissionRate.value = platform?.commissionRate ?? 0;
        if (!form.elements.rawOrder.value)
            form.elements.rawOrder.value = "99301,1";
        form.querySelectorAll("input, select, textarea, button").forEach((element) => {
            element.disabled = !editable;
        });
    }
    ...
    function renderExternalDeliveryIntegrations() {
        ...
        if (!platformList || !mappingList || !importList)
            return;
...
//# sourceMappingURL=external-delivery.js.map