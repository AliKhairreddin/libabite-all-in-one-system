...
export async function verifyStripeWebhookSignature(rawBody, signatureHeader, webhookSecret, options = {}) {
    const parts = signatureParts(signatureHeader);
    const timestamp = Number(parts.get("t")?.[0]);
    const signatures = parts.get("v1") || [];
    const nowMs = Number.isFinite(Number(options.nowMs)) ? Number(options.nowMs) : Date.now();
    const toleranceSeconds = Math.max(0, Math.floor(Number(options.toleranceSeconds) || DEFAULT_STRIPE_SIGNATURE_TOLERANCE_SECONDS));
    ...
    try {
        ...
        const key = await globalThis.crypto.subtle.importKey("raw", encoder.encode(webhookSecret), { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
        const expected = new Uint8Array(await globalThis.crypto.subtle.sign("HMAC", key, encoder.encode(`${timestamp}.${rawBody}`)));
