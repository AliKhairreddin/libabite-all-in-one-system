...
import { getRecipeLineQuantity, getRecipeLineWasteMultiplier, getRecipeMeasure } from "./recipes.js";
...
export function getDefaultProductionProductId(products, selectedProductId = "") {
    ...
    if (productionProducts.some((product) => product.id === selectedProductId))
...
export function getProductionReadiness(draft, options = {}) {
    ...
    if (shortages.length)
        return { ok: false, className: "danger", label: "Missing stock", detail: shortages.map((line) => `${line.ingredient.name} ${formatAmount(line.shortage, line.ingredient.unit)}`).join(", ") };
    if (needsOutputQuantity)
        return { ok: false, className: "warning", label: "Yield needed", detail: "Enter the prepared batch quantity." };
    if (!options.stepsDone || !options.markedDone)
