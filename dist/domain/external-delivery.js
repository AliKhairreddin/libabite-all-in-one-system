import { EXTERNAL_DELIVERY_IMPORT_METHODS, EXTERNAL_DELIVERY_PLATFORM_STATUSES, EXTERNAL_DELIVERY_PLATFORMS } from "../shared/constants.js";
...
export function findExternalProductMapping({ platformId, externalCode = "", externalName = "" } = {}, mappings = []) {
...
export function matchExternalOrderItems(rawItems = [], { platformId, mappings = [], productById }) {
    const matched = [];
    const unmatched = [];
...
export function buildExternalMenuPayload(platform, mappings = [], productById, options = {}) {
...
export function mapInternalOrderStatusToExternalStatus(order) {
    ...
    if (order.status === "Ready")
        return "ready";
    if (["Preparing", "Delayed", "Sent to kitchen"].includes(order.status))
        return "preparing";
    return "accepted";
}
