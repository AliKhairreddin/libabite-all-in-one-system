...
function matchIngredientByValue(ingredients, value) {
    return (ingredients || []).find((ingredient) => (scanMatches(ingredient.id, value)
...
function matchProductByValue(products, value) {
    return (products || []).find((product) => (scanMatches(product.id, value)
        || scanMatches(product.name, value)
        || scanMatches(product.code, value)));
}
function matchTableByValue(tables, tableQrCodes, value) {
    const qrCode = (tableQrCodes || []).find((code) => (scanMatches(code.token, value)
...
function matchUserByValue(users, value) {
    return (users || []).find((user) => (scanMatches(user.id, value)
...
export function resolveScanCode(rawCode, context = {}) {
    ...
    const { ingredients = [], products = [], tableQrCodes = [], tables = [], users = [], locations = [] } = context;
