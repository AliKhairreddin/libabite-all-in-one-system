...
export function getOrdersForCustomer(customer, orders) {
    ...
    return orders
        ...
        if (order.customerId && order.customerId === customer.id)
            return true;
        if (phoneKey && customerPhoneKey(order.customerPhone) === phoneKey)
            return true;
...
export function getManualOrderCustomerDetails(formData, channel, customerById) {
    if (!isPhoneMessageOrder(channel))
        return null;
...
export function upsertCustomerFromOrderDetails(customers, details) {
    ...
    if (!name && !phone && !email && !deliveryAddress && !notes)
        return null;
    ...
    const consentedEmailChanged = Boolean(customer.marketingConsent === true
        && nextEmail
    ...
    if (deliveryAddress)
        customer.addresses = normalizeAddressHistory([deliveryAddress, ...(customer.addresses || [])]);
    if (notes)
