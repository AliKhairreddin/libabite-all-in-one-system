...
export function getReservationDateBounds(policy = {}) {
...
export function reservationDateTimeToUtcMs(date, time, timeZone = "Europe/Amsterdam") {
    if (!isReservationDate(date) || !isReservationTime(time))
        return null;
    const [year, month, day] = date.split("-").map(Number);
...
export function getReservationPolicyValidation(candidate, policy = {}) {
    ...
    if (guests > maxGuests) {
        return reservationPolicyBlock("Party too large", `Online reservations are limited to ${maxGuests} guests. Contact the restaurant for larger groups.`, "Call us");
    }
    ...
    const withinHours = openMinutes !== closeMinutes && (closeMinutes > openMinutes
        ...
        : requestedMinutes >= openMinutes || requestedMinutes < closeMinutes);
    if (!withinHours) {
        return reservationPolicyBlock("Outside booking hours", `Choose an arrival time between ${opensAt} and ${closesAt}.`, "Closed");
    }
    const timeZone = String(policy.timeZone || "Europe/Amsterdam").trim();
    ...
    if (requestedAtMs < nowMs + leadMinutes * 60 * 1000) {
        return reservationPolicyBlock("More notice needed", leadMinutes
            ? `Online requests need at least ${leadMinutes} minutes' notice.`
            : "Choose a future arrival time.", "Too soon");
    }
    const dateBounds = getReservationDateBounds({ ...policy, nowMs, timeZone });
    if (candidate.date > dateBounds.max) {
        return reservationPolicyBlock("Too far ahead", `Online reservations open ${dateBounds.horizonDays} days in advance.`, "Not open yet");
    }
    return { ok: true };
}
...
function getAbsoluteReservationWindow(reservation, turnoverMinutes, timeZone) {
    if (!isReservationDate(reservation?.date) || !isReservationTime(reservation?.time))
        return null;
...
function reservationRecordsOverlap(first, second, turnoverMinutes, timeZone) {
    ...
    if (!sameReservationDate(first?.date, second?.date))
        return false;
...
export function getReservationConflicts(candidate, reservations, turnoverMinutes = RESERVATION_TURNOVER_MINUTES, timeZone = candidate?.timeZone || "Europe/Amsterdam") {
...
function compareTablesForParty(guests) {
    return (first, second) => {
        ...
        if (guests <= 2) {
            ...
            if (firstTwoTop !== secondTwoTop)
                return firstTwoTop - secondTwoTop;
        }
        return firstCapacity - secondCapacity || getTableName(first).localeCompare(getTableName(second));
    };
}
export function getAvailableReservationTables(candidate, tables, reservations, turnoverMinutes = RESERVATION_TURNOVER_MINUTES, timeZone = candidate?.timeZone || "Europe/Amsterdam") {
...
export function getAvailableReservationTable(candidate, tables, reservations, turnoverMinutes = RESERVATION_TURNOVER_MINUTES, timeZone = candidate?.timeZone || "Europe/Amsterdam") {
    return getAvailableReservationTables(candidate, tables, reservations, turnoverMinutes, timeZone)[0] || null;
}
export function getReservationMergeOptions(candidate, tables, reservations, turnoverMinutes = RESERVATION_TURNOVER_MINUTES, timeZone = candidate?.timeZone || "Europe/Amsterdam") {
    const guests = getGuestCount(candidate.guests);
    if (guests < 5)
        return [];
    ...
    return options.sort((first, second) => {
        if (first.sameZone !== second.sameZone)
            return first.sameZone ? -1 : 1;
...
export function getReservationSeatingRecommendation(candidate, tables, reservations, turnoverMinutes = RESERVATION_TURNOVER_MINUTES, timeZone = candidate?.timeZone || "Europe/Amsterdam") {
    const guests = getGuestCount(candidate.guests);
...
export function getReservationIssues(reservation, tables, reservations, turnoverMinutes = RESERVATION_TURNOVER_MINUTES, blocks = [], capacityRules = [], timeZone = reservation?.timeZone || "Europe/Amsterdam") {
...
export function getReservationRequestValidation(candidate, tables, reservations, blocks = [], capacityRules = [], turnoverMinutes = RESERVATION_TURNOVER_MINUTES, policy = {}) {
    const guests = Math.max(1, Math.floor(Number(candidate.guests) || 1));
    ...
    const table = candidate.tableId
        ...
        : getAvailableReservationTable(candidate, tables, reservations, turnoverMinutes, policy.timeZone || candidate?.timeZone || "Europe/Amsterdam");
    if (!table) {
        return {
            ok: false,
            className: "danger",
...
export function getReservationValidation(candidate, tables, reservations, turnoverMinutes = RESERVATION_TURNOVER_MINUTES, blocks = [], capacityRules = [], timeZone = candidate?.timeZone || "Europe/Amsterdam") {
    const guests = Math.max(1, Math.floor(Number(candidate.guests) || 1));
