import { PRECAUTIONARY_ALLERGEN_STATUSES, PRODUCT_ALLERGENS, VAT_OPTIONS, VAT_RATES } from "../shared/constants.js";
...
export function productAllergenSummary(product = {}) {
    ...
    if (!labels.length && precautionaryStatus === "none" && !precautionaryNote)
        return "";
    const parts = [];
    if (labels.length)
        parts.push(`Contains ${labels.join(", ")}`);
    if (precautionaryStatus === "may_contain")
        parts.push(precautionaryNote || "May contain traces of other allergens");
    if (precautionaryStatus === "ask_staff")
        parts.push(precautionaryNote || "Ask staff for allergen details");
    return parts.join(". ");
}
