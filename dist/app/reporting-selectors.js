import { CUSTOMER_QR_CHANNEL, DRIVER_IDLE_STATUS, EXTERNAL_DELIVERY_ORDER_CHANNEL, PHONE_MESSAGE_ORDER_CHANNEL, RESERVATION_ACTIVE_STATUSES, WEBSITE_ORDER_CHANNEL } from "../shared/constants.js";
import { deliveryIsLate, getDeliveryStatus, isActiveDelivery, isDeliveryOrder } from "../domain/delivery.js";
import { formatShiftHours, getShiftMetrics, toDateInputString } from "../domain/scheduling.js";
import { normalizeOrderType } from "../domain/orders.js";
import { state } from "./state.js";
import { customerById, ingredientById, productById, supplierById, tableById } from "./entities.js";
import { getOrderTotal, isOrderPaid, normalizeOrderItems } from "./order-selectors.js";
import { getKitchenSlaSummary, getOpenTickets, getTicketAgeMinutes, getTicketSla } from "./kitchen-selectors.js";
import { getIngredientStatus, getLowStockIngredients, getStockRequirementsForItems, getSupplierOrderDrafts, getSupplierOrderTotal } from "./inventory-selectors.js";
import { getProductCost, getProductMarginProfile } from "./recipe-selectors.js";
...
function orderTypeBucket(order) {
    ...
    if (channel === "Dine-in")
        return "dineIn";
    if (channel === CUSTOMER_QR_CHANNEL)
        return "qr";
    if (channel === "Takeaway")
        return "takeaway";
    if (channel === "Delivery")
        return "delivery";
    if (channel === WEBSITE_ORDER_CHANNEL)
        return "website";
    if (channel === EXTERNAL_DELIVERY_ORDER_CHANNEL)
        return "externalApps";
    if (channel === PHONE_MESSAGE_ORDER_CHANNEL && order.fulfillment === "Delivery")
        return "delivery";
    return "takeaway";
}
...
function getLateOrderRows() {
    ...
    getOpenTickets().forEach((ticket) => {
        ...
        if (!["warning", "escalated", "delayed"].includes(sla.state))
            return;
...
//# sourceMappingURL=reporting-selectors.js.map