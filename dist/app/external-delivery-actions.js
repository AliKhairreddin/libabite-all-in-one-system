import { EXTERNAL_DELIVERY_ORDER_CHANNEL, EXTERNAL_DELIVERY_PLATFORMS } from "../shared/constants.js";
import { buildExternalMenuPayload, calculateExternalCommission, externalImportMethodLabel, externalPlatformName, matchExternalOrderItems, mapInternalOrderStatusToExternalStatus, normalizeExternalCommissionRate, normalizeExternalImportMethod, normalizeExternalPlatformId, normalizeExternalPlatformStatus, parseExternalOrderLines } from "../domain/external-delivery.js";
...
export function createExternalDeliveryRuntime(deps) {
    const { can, currentUser, getOrderTotal, normalizeOrderItems, productById, render, sendOrderToKitchen, showToast, upsertCustomerFromOrderDetails, validateOrderForKitchen } = deps;
    ...
    function saveExternalPlatformRecord(formData) {
        ...
        if (existing)
            ...
            state.externalPlatforms.push(record);
        saveState();
        render();
    ...
    function saveExternalProductMapping(formData) {
        ...
        const existing = state.externalProductMappings.find((mapping) => mapping.id === mappingId)
            || state.externalProductMappings.find((mapping) => {
                if (mapping.platformId !== platformId)
                    return false;
                if (externalCode && sameText(mapping.externalCode, externalCode))
                    return true;
        ...
        if (existing)
            ...
            state.externalProductMappings.push(mapping);
        saveState();
        render();
...
//# sourceMappingURL=external-delivery-actions.js.map