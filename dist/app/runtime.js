...
import { getProductionFieldName, getProductionOutputDefault, roundMoneyValue } from "../domain/production.js";
import { getReservationWindowLabel } from "../domain/reservations.js";
import { convertWasteQuantityToStockUnits, getIngredientPrimaryLocation, getWasteCost, getWasteUnitOptionsForIngredient, normalizeInventoryLocationName, normalizeKitchenStation, normalizeMarginPercent, normalizeRecipeAppliesTo, normalizeStockQuantity, normalizeWasteReason, normalizeWasteUnitType, unitTypeDefinition } from "../data/normalize.js";
...
import { can, canView, currentRole, currentRoleKey, currentUser, ensureActiveViewAccess, roleDefinition, visibleViews } from "./permissions.js";
import { customerById, ingredientById, money, orderById, productById, supplierById, tableById } from "./entities.js";
import { findCustomerByPhone, findCustomerBySearchValue, getAddressHistoryForCustomer, getCustomerOptionLabel, getCustomerPrimaryAddress, getFavoriteItemsForCustomer, getManualOrderCustomerDetails, getOrdersForCustomer, upsertCustomerFromOrderDetails } from "./customer-selectors.js";
import { getAvailableReservationTable, getReservationIssues, getReservationRequestValidation, getReservationValidation } from "./reservation-selectors.js";
import { convertActualUsageToStockUnits, convertRecipeLineToStockUnits, getLineCost, getProductCost, getProductGrossMargin, getProductMargin, getProductMarginProfile, getRecipeLineQuantity, getRecipeLineWasteMultiplier, getRecipeMeasure, getRecipeMeasureOptionsForIngredient, getRecipeUsageLabel, productAvailabilityLabel, productHasConditionalRecipeLines, recipeLineAppliesToOrder } from "./recipe-selectors.js";
import { getDefaultProductionProductId, getProductionExecutionDraft, getProductionOutputUnitType, getProductionProducts, getProductionReadiness } from "./production-selectors.js";
import { formatLocationOptionLabel, formatWasteQuantity, getActiveSupplierOrder, getAllInventoryLocations, getIngredientLocationRows, getIngredientStatus, getLowStockIngredients, getOverStockIngredients, getProductAvailability, getStockRequirementsForItems, getStockShortages, getSupplierKey, getSupplierMinimumOrderGap, getSupplierOrderDrafts, getSupplierOrderPayload, getSupplierOrderQuantity, getSupplierOrderTotal, getWasteReportSummary, inventoryActionLabel, supplierForIngredient, wasteUnitLabel } from "./inventory-selectors.js";
import { getKitchenSlaSummary, getOpenTickets, getSlaSummaryLabel, getStationNames, getTicketOrderAgeMinutes, getTicketPriority, getTicketSla, getTicketStatusLabel, ticketStatusClass } from "./kitchen-selectors.js";
import { fulfillmentLabel, getCurrentOrderContext, getItemCount, getItemsTotal, getKitchenOrderProgressSummary, getOrderFulfillmentMeta, getOrderPaidByName, getOrderPaymentSummary, getOrderStaffName, getOrderSubtotalExcludingVat, getOrderTotal, getOrderVatBreakdown, getOrderableProductsForContext, getVatLabel, isOrderPaid, normalizeOrderItems, orderLocationLabel, orderStatusClass, orderTypeLabel } from "./order-selectors.js";
import { getOrderCompletionToast } from "./order-toasts.js";
import { getManagementDashboardData } from "./reporting-selectors.js";
const { assignQrCode, createTableQrCode, getCustomerOrderingSession, getCustomerQrSession, getQrOrderUrl, getStaffUrl, getWebsiteOrderSession, getWebsiteReservationSession, getWebsiteReservationUrl, openQrCustomerUrl, regenerateQrCode, toggleQrCode } = createQrRuntime({
    can,
    render: () => render(),
    showToast,
    tableById
});
const { addDeliveryNote, assignDeliveryOrderToDriver, assignDriverToDeliveryOrder, canManageDeliveryOperations, currentDriverRecord, currentUserCanUpdateDelivery, driverById, markDeliveryCashCollected, updateDeliveryStatus, uploadDeliveryProof } = createDeliveryRuntime({
    currentRoleKey,
    currentUser,
    isOrderPaid,
    orderById,
...
const { addCustomerCartItem, adjustCustomerCartItem, getCustomerCartItems, getCustomerCartTotal, getCustomerOrderContext, removeCustomerCartItem, setWebsiteFulfillment, startNewCustomerOrder, submitCustomerQrOrder, submitWebsiteOrder } = createCustomerOrderingRuntime({
...
const { importExternalOrder, pushExternalOrderStatus, pushMenuToExternalPlatform, saveExternalPlatformRecord, saveExternalProductMapping, toggleExternalProductMapping } = createExternalDeliveryRuntime({
    can,
    currentUser,
    getOrderTotal,
    normalizeOrderItems,
...
const { alertCard, getSelectedPaymentMethodFromAction, orderCard, orderItemDetailText, paymentCaptureHtml } = createOrderCardsUi({
    can,
    formatStockAmount,
    fulfillmentLabel,
    getActiveSupplierOrder,
...
const { renderInventory, renderInventoryActionForm, renderProductManagement, renderPurchasedProductForm, renderSellableProductForm, renderSellableRecipeCostPreview, renderWasteForms, renderWasteReport, renderWasteTracking } = createInventoryUi({
    can,
    alertCard,
...
const { addSellableRecipeLine, createPurchasedProduct, createSellableProduct, removeSellableRecipeLine, togglePurchasedProduct, toggleSellableProduct, updateIngredientPurchasePrice } = createProductActionsRuntime({
    can,
...
const { getCurrentUserProcedures, getProcedureStepProgress, procedureAssignedToUser, procedureById, procedurePeriodStatus, procedureStepsComplete, renderProcedureFormControls, renderProcedures, renderProductionRecipeFields, updateProductionCostPreview } = createProceduresUi({
    can,
    currentRole,
    currentUser,
    emptyState,
...
const { createProcedure, promptAndRecordProcedureStatus, recordProcedureCompletion, setProcedureStepProgress } = createProcedureActionsRuntime({
    can,
    currentUser,
...
const { loadCustomerIntoManualOrder, renderManualOrderControls, renderOrderBuilder, renderProductsInSelects } = createOrderBuilderUi({
    can,
    customerById,
    emptyState,
    findCustomerByPhone,
...
const { applyScannedInventoryAction, scanCode } = createScanningRuntime({
    can,
    canView,
    getAllInventoryLocations,
    ingredientById,
...
const { renderQrCodeManagement, renderReservationPlanner, renderReservations, renderSettings } = createSettingsUi({
    can,
    emptyState,
...
const { addReservation, addReservationBlock, cancelReservationEdit, deleteReservationBlock, deleteReservationCapacityRule, saveReservationCapacityRule, selectReservationForEdit, submitWebsiteReservation, updateReservationStatus } = createReservationActionsRuntime({
    can,
    getAvailableReservationTable,
...
const { createStaffUser, saveRestaurantSettings } = createAdminActionsRuntime({
    can,
    render: () => render(),
    roleDefinition,
    showToast
});
const { login, logout, setView } = createSessionActionsRuntime({
    canView,
    render: () => render(),
    roleDefinition,
    showToast
