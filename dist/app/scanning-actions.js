...
export function createScanningRuntime(deps) {
    ...
    const { can, canView, getAllInventoryLocations, ingredientById, productById, render, renderInventoryActionForm, renderOrderBuilder, renderProductsInSelects, renderWasteForms, showToast, tableById } = deps;
    ...
    function selectWasteProduct(ingredientId) {
        const wasteForm = [...document.querySelectorAll("[data-waste-form]")][0];
        const productSelect = wasteForm?.querySelector("[data-waste-product]");
        if (productSelect)
            productSelect.value = ingredientId;
        renderWasteForms();
    ...
    function openIngredientInventory(result, action = "add") {
        ...
        render();
        if (action === "waste")
            selectWasteProduct(result.targetId);
        else
            selectInventoryActionProduct(result.targetId, action);
    ...
    function openTableOrder(result) {
        ...
        if (orderForm?.elements.channel)
            orderForm.elements.channel.value = CUSTOMER_QR_CHANNEL;
        renderProductsInSelects();
        if (orderForm?.elements.tableId)
            orderForm.elements.tableId.value = table.id;
        renderOrderBuilder();
    ...
    function scanCode(formData) {
        ...
        if (result.targetKind === "ingredient")
            return openIngredientInventory(result);
        if (result.targetKind === "product")
            return openRecipe(result);
        if (result.targetKind === "table")
            return openTableOrder(result);
        if (result.targetKind === "location")
            return openStorageLocation(result);
    ...
    function applyScannedInventoryAction(action) {
        ...
        render();
        if (actionId === "waste")
            selectWasteProduct(scan.targetId);
        else
            selectInventoryActionProduct(scan.targetId, actionId);
...
//# sourceMappingURL=scanning-actions.js.map