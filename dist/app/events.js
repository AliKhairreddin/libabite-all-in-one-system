...
export function bindAppEvents(handlers) {
    ...
    document.addEventListener("click", (event) => {
        ...
        if (assignDeliveryDriver)
            assignDeliveryOrderToDriver(assignDeliveryDriver.dataset.assignDeliveryDriver);
        const deliveryStatus = event.target.closest("[data-delivery-status][data-delivery-order]");
        if (deliveryStatus)
            updateDeliveryStatus(deliveryStatus.dataset.deliveryOrder, deliveryStatus.dataset.deliveryStatus);
        const deliveryCash = event.target.closest("[data-delivery-cash]");
        if (deliveryCash)
            markDeliveryCashCollected(deliveryCash.dataset.deliveryCash);
        const deliveryNote = event.target.closest("[data-add-delivery-note]");
        if (deliveryNote)
            addDeliveryNote(deliveryNote.dataset.addDeliveryNote);
        ...
        if (websiteFulfillment)
            setWebsiteFulfillment(websiteFulfillment.dataset.websiteFulfillment);
    ...
    document.querySelector("#customerSelect")?.addEventListener("change", (event) => {
        if (event.currentTarget.value)
            loadCustomerIntoManualOrder(event.currentTarget.value);
        else
            renderManualOrderControls();
    });
    document.querySelector("#customerSearch")?.addEventListener("change", (event) => {
        const customer = findCustomerBySearchValue(event.currentTarget.value);
        if (customer)
            loadCustomerIntoManualOrder(customer.id);
        else
            renderManualOrderControls();
    });
    ...
    document.querySelector("#resetDemoBtn").addEventListener("click", () => {
        ...
        if (nextState.users.some((user) => user.id === previousUserId))
            nextState.currentUserId = previousUserId;
        saveState();
        render();
