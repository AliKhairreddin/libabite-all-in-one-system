...
import { addDays, getShiftAttendanceStatus, getWeekStartDate, isShiftTime, normalizeScheduleRole, normalizeScheduleStation, normalizeShiftDate, sortShiftsByDateTime, toDateInputString } from "../domain/scheduling.js";
import { saveState, state } from "./state.js";
export function createSchedulingRuntime(deps) {
    const { can, currentRoleKey, currentUser, render, roleDefinition, showToast } = deps;
    ...
    function selectStaffShiftForEdit(shiftId) {
        ...
        if (submitButton)
            submitButton.textContent = "Save Shift";
    ...
    function cancelStaffShiftEdit() {
        ...
        if (elements.role)
            elements.role.value = "Front";
        if (elements.station)
            elements.station.value = "Restaurant floor";
    ...
    function moveScheduleWeek(direction) {
        ...
        if (direction === "previous")
            state.scheduleWeekStart = addDays(currentWeekStart, -7);
        else if (direction === "next")
            state.scheduleWeekStart = addDays(currentWeekStart, 7);
        else
            state.scheduleWeekStart = getWeekStartDate(toDateInputString());
        saveState();
        render();
    }
...
//# sourceMappingURL=scheduling-actions.js.map