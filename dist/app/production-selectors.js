...
import { convertWasteQuantityToStockUnits, getWasteUnitOptionsForIngredient, unitTypeDefinition } from "../data/normalize.js";
import { getDefaultProductionProductId as getDefaultProductionProductIdFromList, getProductionExecutionDraft as getProductionExecutionDraftFromValues, getProductionOutputUnitType as getProductionOutputUnitTypeForIngredient, getProductionProducts as getProductionProductsFromList, getProductionReadiness as getProductionReadinessForDraft } from "../domain/production.js";
import { state } from "./state.js";
import { ingredientById, productById } from "./entities.js";
import { convertActualUsageToStockUnits, convertRecipeLineToStockUnits } from "./recipe-selectors.js";
...
export function getProductionStepCheckboxes(form = document.querySelector("#productionForm")) {
    return Array.from(form?.querySelectorAll("[data-production-step]") || []);
}
export function productionStepsComplete(form = document.querySelector("#productionForm")) {
    const steps = getProductionStepCheckboxes(form);
    return steps.length ? steps.every((step) => step.checked) : false;
}
export function productionMarkedComplete(form = document.querySelector("#productionForm")) {
...
//# sourceMappingURL=production-selectors.js.map