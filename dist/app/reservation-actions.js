...
import { isReservationDate, isReservationTime, normalizeReservationStatus } from "../domain/reservations.js";
...
export function createReservationActionsRuntime(deps) {
    const { can, getAvailableReservationTable, getReservationRequestValidation, getReservationValidation, render, renderReservationPlanner, renderWebsiteReservationScreen, showToast, tableById } = deps;
    function reservationById(id) {
        return state.reservations.find((reservation) => reservation.id === id);
    }
    function reservationFromForm(formData, fallback = {}) {
    ...
    function selectReservationForEdit(reservationId) {
        ...
        if (!reservationById(reservationId))
            return;
